...
public class CharacterMotor : MonoBehaviour
{
    ...
    public CharacterMotorMovement movement; //= CharacterMotorMovement();
    ...
    public class CharacterMotorMovement
    {
        ...
        double maxForwardSpeed = 10.0;
        double maxSidewaysSpeed = 10.0;
        double maxBackwardsSpeed = 10.0;

        // Curve for multiplying speed based on slope (negative = downwards)
        AnimationCurve slopeSpeedMultiplier = AnimationCurve(Keyframe(-90, 1), Keyframe(0, 1), Keyframe(90, 0));

        // How fast does the character change speeds?  Higher is faster.
        double maxGroundAcceleration = 30.0;
        double maxAirAcceleration = 20.0;
    ...
    class CharacterMotorJumping
    {
        ...
        double extraHeight = 4.1;

        // How much does the character jump out perpendicular to the surface on walkable surfaces?
        // 0 means a fully vertical jump and 1 means fully perpendicular.
        double perpAmount = 0.0;

        // How much does the character jump out perpendicular to the surface on too steep surfaces?
        // 0 means a fully vertical jump and 1 means fully perpendicular.
        double steepPerpAmount = 0.5;
    ...
    class CharacterMotorSliding
    {
        ...
        double slidingSpeed = 15;

        // How much can the player control the sliding direction?
        // If the value is 0.5 the player can slide sideways with half the speed of the downwards sliding speed.
        double sidewaysControl = 1.0;

        // How much can the player influence the sliding speed?
        // If the value is 0.5 the player can speed the sliding up to 150% or slow it down to 50%.
        double speedControl = 0.4;
    }
    ...
    public void UpdateFunction()
    {
        ...
        {
            double projectedNewVelocity = Vector3.Dot(newHVelocity, oldHVelocity) / oldHVelocity.sqrMagnitude;
            movement.velocity = (double)oldHVelocity * Mathf.Clamp01(projectedNewVelocity) + movement.velocity.y * Vector3.up;
        }
    ...
    private Vector3 ApplyInputVelocityChange(Vector3 velocity)
    {
        ...
        double maxVelocityChange = GetMaxAcceleration(grounded) * Time.deltaTime;
    ...
    double GetMaxAcceleration(bool grounded)
    {
    ...
    float CalculateJumpVerticalSpeed(double targetJumpHeight)
    {
    ...
    private double MaxSpeedInDirection(Vector3 desiredMovementDirection)
    {
        ...
        else
        {
            ...
            double length = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.maxSidewaysSpeed;
            return length;
        }
    }
